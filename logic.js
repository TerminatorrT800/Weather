...
document.querySelector(".satBTN").addEventListener("click", renderTable);
document
  .querySelector(".trenutnoBTN")
  .addEventListener("click", trenutnoPrikaz);

async function getWeather(location) {
  try {
    const data = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}?unitGroup=us&key=RBMWSMLNFZEZX6RXBZQXHGAWZ&contentType=json`,
      {
        method: "GET",
      }
    );
    ...
    response.days.forEach((el) => {
      el.hours.forEach((elH) => {
        let hour = new Hours(elH.datetime, elH.conditions, elH.temp, elH.icon);
        hours.push(hour);
      });
      let day = new Day(
        el.datetime,
